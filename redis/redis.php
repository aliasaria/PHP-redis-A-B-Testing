...
class Redis {
    ...
    private function read($len=1024) {
        if ($s = fgets($this->_sock))
            return $s;
        $this->disconnect();
        trigger_error("Cannot read from socket.", E_USER_WARNING);
    }
    ...
    private function get_response() {
        ...
        switch ($c) {
            ...
            case ':':
                ...
                if ((string)$i != $data)
                    trigger_error("Cannot convert data '$c$data' to integer", E_USER_WARNING);
                return $i;
            case '$':
            ...
            case '*':
                $num = (int)$data;
                if ((string)$num != $data)
                    trigger_error("Cannot convert multi-response header '$data' to integer", E_USER_WARNING);
                $result = array();
                for ($i=0; $i<$num; $i++)
    ...
    private function get_bulk_reply($data=null) {
        ...
        if ((string)$bulklen != $data)
            trigger_error("Cannot convert bulk read header '$c$data' to integer", E_USER_WARNING);
        if ($c != '$')
            trigger_error("Unkown response prefix for '$c$data'", E_USER_WARNING);
        $buffer = '';
        while ($bulklen) {
